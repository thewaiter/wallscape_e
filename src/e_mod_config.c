...
static void
_pan_unhilight(Evas_Object *obj EINA_UNUSED, Item *it)
{
//   Smart_Data *sd = evas_object_smart_data_get(obj);
...
static void
_pan_hilight(Evas_Object *obj EINA_UNUSED, Item *it)
{
   Eina_List *l;
   Item *it2;
   Smart_Data *sd = evas_object_smart_data_get(obj);
...
/*
static void
_item_down(void *data EINA_UNUSED, Evas *e EINA_UNUSED, Evas_Object *obj EINA_UNUSED, void *event_info EINA_UNUSED)
{
...
static void
_apply(void *data, void *data2 EINA_UNUSED)
{
   Info *info = data;
   ...
   if (info->mode == 0)
   ...
   else if (info->mode == 1)
     {
        ...
        e_bg_add(info->zone_num, info->desk_x, info->desk_y,
                 info->bg_file);
     }
   else
     {
...
static void
_close(void *data EINA_UNUSED, void *data2 EINA_UNUSED)
{
   wp_conf_hide();
}

static void
_ok(void *data, void *data2 EINA_UNUSED)
{
   _apply(data, data2);
   wp_conf_hide();
}
...
static void
_wp_add(void *data, void *data2 EINA_UNUSED)
{
   Info *info = data;
...
static void
_wp_delete(void *data, void *data2 EINA_UNUSED)
{
   Info *info = data;
...
static void
_wp_changed(void *data, Evas_Object *obj EINA_UNUSED, void *event_info EINA_UNUSED)
{
   Info *info = data;
...
wp_browser_new()
{
   ...
   ecore_evas_name_class_set(e_win_ee_get(win), "E", "_config::appearance/wallscape");
   //~ e_win_resize_callback_set(win, _resize);
   //~ e_win_delete_callback_set(win, _delete);

   // bg + container
   
   //~ info->bg = edje_object_add(info->win);
   info->bg =edje_object_add (e_win_evas_win_get(evas));
   ...
   info->box = e_widget_list_add(evas, 1, 1);

   // ok button
   info->button = e_widget_button_add(evas, D_("OK"), NULL,
                                      _ok, info, NULL);
   e_widget_list_object_append(info->box, info->button, 1, 0, 0.5);

   // apply button
   info->button = e_widget_button_add(evas, D_("Apply"), NULL,
                                      _apply, info, NULL);
   e_widget_list_object_append(info->box, info->button, 1, 0, 0.5);

   // close button
   info->button = e_widget_button_add(evas, D_("Close"), NULL,
                                      _close, info, NULL);
   e_widget_list_object_append(info->box, info->button, 1, 0, 0.5);
   ...
   // preview
   info->preview = e_livethumb_add(evas);
   ...
   ob = e_widget_list_add(evas, 0, 1);

   o = e_widget_list_add(evas, 1, 0);

   rg = e_widget_radio_group_new(&(info->mode));
   o2 = e_widget_radio_add(evas, D_("All Desktops"), 0, rg);
   evas_object_smart_callback_add(o2, "changed", _wp_changed, info);
   e_widget_list_object_append(o, o2, 1, 0, 0.5);
   e_widget_disabled_set(o2, zone->desk_x_count * zone->desk_y_count < 2);
   evas_object_show(o2);

   o2 = e_widget_radio_add(evas, D_("This Desktop"), 1, rg);
   evas_object_smart_callback_add(o2, "changed", _wp_changed, info);
   e_widget_list_object_append(o, o2, 1, 0, 0.5);
   evas_object_show(o2);

   o2 = e_widget_radio_add(evas, D_("This Screen"), 2, rg);
   evas_object_smart_callback_add(o2, "changed", _wp_changed, info);
   e_widget_list_object_append(o, o2, 1, 0, 0.5);
   if (!(e_comp_zone_number_get(0) ||
         (e_comp_zone_number_get(1))))
     e_widget_disabled_set(o2, EINA_TRUE);
   evas_object_show(o2);

   o2 = e_widget_check_add(evas, D_("Use Theme Wallpaper"),
                           &info->use_theme_bg);
   evas_object_smart_callback_add(o2, "changed", _wp_changed, info);
   info->theme_bg = o2;
   e_widget_list_object_append(o, o2, 1, 0, 0.5);
    ...
    * 
   o2 =  e_widget_button_add(info->win, D_("Add"), NULL,
                             _wp_add, info, NULL);
   e_widget_list_object_append(o, o2, 1, 0, 0.5);
   evas_object_show(o2);

   o2 =  e_widget_button_add(info->win, D_("Delete"), NULL,
